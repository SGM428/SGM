...
def _parse_wallet_text_legacy(content: str) -> dict[int, int]:
    """예전(텍스트 본문 저장) 형식 파서. 마이그레이션 및 하위호환용으로만 남겨둠."""
...
async def _load_balances(existing_msg: Optional[discord.Message]) -> dict[int, int]:
    ...
    첨부파일이 없으면(마이그레이션 전, 예전 텍스트 형식 메시지) 본문에서 예전 방식으로 파싱한다."""
    if existing_msg is None:
        return {}
    if existing_msg.attachments:
        data = await existing_msg.attachments[0].read()
        return _parse_wallet_json(data)
    return _parse_wallet_text_legacy(existing_msg.content)
...
def _parse_record_text_legacy(content: str) -> dict[str, tuple[int, int]]:
    """예전(텍스트 본문 저장) 형식 파서. 마이그레이션 및 하위호환용으로만 남겨둠."""
    records: dict[str, tuple[int, int]] = {}
    for line in content.splitlines():
...
async def _load_records(existing_msg: Optional[discord.Message]) -> dict[str, tuple[int, int]]:
    ...
    return _parse_record_text_legacy(existing_msg.content)
...
@bot.tree.command(
    name="먹물코인마이그레이션",
    description="[관리자] 예전 텍스트 형식으로 저장된 지갑 데이터를 새 파일 첨부 형식으로 옮깁니다. 딱 한 번만 실행하면 돼요.",
)
@app_commands.checks.has_permissions(manage_guild=True)
async def migrate_wallet(interaction: discord.Interaction):
    channel = await _get_wallet_channel(interaction.guild_id, interaction.client)
    if channel is None:
        await interaction.response.send_message("먼저 `/먹물코인채널설정`으로 채널을 지정해주세요.", ephemeral=True)
        return

    async with _get_wallet_lock(interaction.guild_id):
        existing_msg = await _find_wallet_message(interaction.client, channel, interaction.guild_id)
        if existing_msg is None:
            await interaction.response.send_message("옮길 지갑 데이터가 없어요 (기존 메시지를 못 찾았어요).", ephemeral=True)
            return

        if existing_msg.attachments:
            await interaction.response.send_message("이미 새 형식(파일 첨부)으로 저장돼있어서 옮길 게 없어요.", ephemeral=True)
            return

        balances = await _load_balances(existing_msg)  # 첨부파일이 없으니 예전 텍스트 형식으로 파싱됨
        if not balances:
            await interaction.response.send_message("기존 메시지에서 지갑 데이터를 읽지 못했어요. 형식을 확인해주세요.", ephemeral=True)
            return

        try:
            await _replace_wallet_message(interaction.client, channel, interaction.guild_id, balances)
        except discord.Forbidden:
            await interaction.response.send_message(
    ...
    await interaction.response.send_message(f"총 {len(balances)}명의 지갑 데이터를 새 형식으로 옮겼어요.", ephemeral=True)
...
@bot.tree.command(
    name="전적마이그레이션",
    description="[관리자] 예전 텍스트 형식으로 저장된 전적을 새 파일 첨부 형식으로 옮깁니다. 딱 한 번만 실행하면 돼요.",
)
@app_commands.checks.has_permissions(manage_guild=True)
async def migrate_records(interaction: discord.Interaction):
    channel = await _get_record_channel(interaction)
    if channel is None:
        await interaction.response.send_message(
            "먼저 /내전전적채널설정으로 전적을 기록할 채널을 지정해주세요.", ephemeral=True
    ...
    async with _get_record_lock(interaction.guild_id):
        existing_msg = await _find_record_message(interaction.client, channel, interaction.guild_id)
        if existing_msg is None:
            await interaction.response.send_message("옮길 전적 기록이 없어요 (기존 메시지를 못 찾았어요).", ephemeral=True)
            return
        ...
        records = await _load_records(existing_msg)  # 첨부파일이 없으니 예전 텍스트 형식으로 파싱됨
        if not records:
            await interaction.response.send_message("기존 메시지에서 전적 데이터를 읽지 못했어요. 형식을 확인해주세요.", ephemeral=True)
            return
    ...
    await interaction.response.send_message(f"총 {len(records)}명의 전적을 새 형식으로 옮겼어요.", ephemeral=True)
