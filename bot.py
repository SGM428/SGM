...
- /내전변수확인 : [관리자 전용, 나만 보기] 이 채널의 매치 메모리 상태 확인 + 초기화 버튼
"""
...
def assign_lines_with_pins(
    players: list[PlayerInfo], pinned: dict[str, str], hard_pinned: Optional[dict[str, str]] = None
) -> dict[int, str]:
    """
...
# (active_matches 메모리에 있는 상태를 읽고 고쳐서 그 결과 메시지를 다시 편집함.
#  신청 시 적어낸 주라인/부라인 선호도까지 그대로 들고 있어서 /내전다시섞기 등에서 활용 가능)
# ---------------------------------------------------------------------------
...
def _locked_names(stored: dict) -> set:
    """/내전라인고정·라인교체·팀묶기·팀분리·라인요청 등으로 이미 조건이 걸려있는 닉네임(소문자) 집합."""
    names = set(n.strip().lower() for n in stored.get("fixed_lines", {}))
    for group in stored.get("same_groups", []):
        names.update(n.strip().lower() for n in group)
    for a, b in stored.get("diff_pairs", []):
...
def _build_match_debug_text(match: dict) -> str:
    ...
    for team in ("A", "B"):
        ...
        for i in members:
            p = players[i]
            lines_out.append(f"- {line_assignment[i]}: {p.name} ({p.tier_text})")
        lines_out.append("")
...
class MatchDebugView(discord.ui.View):
    MAX_ITEM_BUTTONS = 20  # 전체 초기화 버튼 자리까지 포함해 25개 제한 안에 여유 있게
    ...
    def __init__(self, channel_id: int, match: dict):
        ...
        for i, (kind, key, _label) in enumerate(items[: self.MAX_ITEM_BUTTONS], start=1):
            self.add_item(self._make_item_button(i, kind, key))

        clear_btn = discord.ui.Button(label="🗑️ 전체 초기화", style=discord.ButtonStyle.danger, row=4)
        clear_btn.callback = self._clear_all
        self.add_item(clear_btn)

    def _make_item_button(self, number: int, kind: str, key) -> discord.ui.Button:
        button = discord.ui.Button(label=f"❌{number}", style=discord.ButtonStyle.secondary)
    ...
    async def _clear_all(self, interaction: discord.Interaction):
        if self.channel_id in active_matches:
            ...
            await interaction.response.edit_message(
                content="🗑️ 이 채널의 매치 메모리를 초기화했어요.", view=None
            )
        else:
            await interaction.response.edit_message(content="이미 데이터가 없어요.", view=None)
...
async def swap_lines(interaction: discord.Interaction, 닉네임1: str, 닉네임2: str):
    ...
    result = solve_team_split(players, line_assignment)
    if result is None:
    ...
    match["constraints"]["fixed_lines"][players[idx_a].name] = line_assignment[idx_a]
    match["constraints"]["fixed_lines"][players[idx_b].name] = line_assignment[idx_b]
...
async def request_line(interaction: discord.Interaction, 닉네임: str, 라인: app_commands.Choice[str]):
    ...
    locked = _locked_names(match["constraints"])
    swappable = [i for i in occupants if players[i].name.strip().lower() not in locked]

    if occupants and not swappable:
        await interaction.response.send_message(
            f"{target_line} 라인에 있는 사람들이 전부 이미 다른 조건으로 고정되어 있어서 옮길 수 없어요.",
            ephemeral=True,
        )
        return

    best = None
    for occ in swappable:
        ...
        if best is None or diff < best[0]:
            best = (diff, occ, trial, result)

    if best is None:
        await interaction.response.send_message("이동시킬 방법을 찾지 못했어요.", ephemeral=True)
        return

    _, swapped_idx, new_line_assignment, (team_a, team_b, score_a, score_b) = best
    swapped_name = players[swapped_idx].name
    original_line = line_assignment[idx]
    if not await _safe_edit_match_message(interaction, msg, format_result(players, team_a, team_b, score_a, score_b)):
        return

    match["line_assignment"] = new_line_assignment
    match["team_of"] = {i: "A" for _, i in team_a} | {i: "B" for _, i in team_b}
    match["constraints"]["fixed_lines"][players[idx].name] = target_line
    await interaction.response.send_message(
...
@bot.tree.command(name="내전변수확인", description="[관리자] 이 채널에 저장된 매치 메모리 상태를 확인하고 필요하면 초기화합니다.")
@app_commands.checks.has_permissions(manage_guild=True)
