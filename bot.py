...
async def blackjack(ctx: commands.Context, bet: int):
    ...
    if _bj_is_blackjack(player_hand):
        for child in view.children:
            child.disabled = True
        view.stop()
        outcome = "push" if _bj_is_blackjack(dealer_hand) else "blackjack"

        settlement, error = await _settle_bet(ctx.guild.id, ctx.author.id, bet, outcome)
        if error == "forbidden":
            footer = f"{_BJ_OUTCOME_TEXT[outcome]} — ⚠️ 잔액 반영 실패 (봇 권한 확인 필요)"
        elif settlement is None:
            footer = f"{_BJ_OUTCOME_TEXT[outcome]}"
        else:
            net, new_balance = settlement
            sign = "+" if net >= 0 else ""
            footer = f"{_BJ_OUTCOME_TEXT[outcome]} — {sign}{net:,} 먹물코인 (현재 잔액 {new_balance:,})"
...
class RPSAIChoiceView(discord.ui.View):
...
class RPSChallengeView(discord.ui.View):
    ...
    async def challenge_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if channel is None:
            ...
            await interaction.response.send_message("먼저 관리자가 `/먹물코인채널설정`으로 채널을 지정해야 해요.", ephemeral=True)
            return
        ...
        try:
            ...
            self.accepted = False
            await interaction.response.send_message("지갑 채널에 쓸 권한이 없어요. 봇 권한을 확인해주세요.", ephemeral=True)
            return
        ...
        await interaction.response.edit_message(content=content, embed=duel_view.build_embed(), view=duel_view)
