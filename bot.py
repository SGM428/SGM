...
class PlayerInfo:
    ...
    # /비밀추가로 등록된, 밸런싱 계산에만 쓰이는 비공개 실제 티어 (표기에는 영향 없음)
    secret_tier: Optional[str] = None
    secret_division: Optional[str] = None

    @property
    def score(self) -> int:
        # LP는 표기/랭킹에만 쓰이고 팀 밸런싱 점수에는 영향을 주지 않음
        if self.secret_tier is not None:
            return tier_score(self.secret_tier, self.secret_division)
        return tier_score(self.tier, self.division)

    @property
    def tier_text(self) -> str:
        # 비밀 티어가 있어도 표기는 항상 신청 시점에 적어낸 티어 그대로 보여줌
...
async def on_ready():
    ...
    if not _persistent_views_registered:
        # on_ready는 재연결될 때마다 다시 불릴 수 있어서, 플래그로 최초 1회만 등록되게 함
        # (등록 자체가 누적돼도 당장 오작동하진 않지만, 불필요하게 계속 쌓이는 걸 막기 위함)
    ...
    if DEV_GUILD_ID:
        try:
            ...
            # .env의 DEV_GUILD_ID가 숫자가 아니면 여기서 죽이지 않고, 경고만 남기고 전역 동기화로 넘어감
            print(f"[경고] DEV_GUILD_ID 값이 올바른 서버 ID가 아니에요: {DEV_GUILD_ID!r} — 전역 동기화로 진행할게요.")
...
async def on_member_remove(member: discord.Member):
    ...
    embed = discord.Embed(
        title="👋 서버 퇴장",
        description=f"{member.mention} (**{member.display_name}**)님이 서버를 나갔어요.",
...
# 길드 ID -> 그 서버의 전적 읽기/계산/저장을 한 번에 하나씩만 처리하게 만드는 잠금
# (서로 다른 채널에서 결과입력/전적수정이 거의 동시에 일어나면 나중에 쓴 쪽이 먼저 쓴 갱신을
#  덮어써서 조용히 날려버릴 수 있어서, 지갑과 동일한 방식으로 잠금을 걺)
record_locks: dict[int, asyncio.Lock] = {}

# 이미 전적에 반영된 "결과 입력" 메시지 ID 집합. 버튼 자체는 모달을 여는 순간 바로 비활성화되지
# 않아서, 두 명이 거의 동시에 누르면 둘 다 모달을 열고 제출할 수 있음. 실제로 전적에 반영되기
# 직전(동기적으로, await 없이)에 여기 걸러서 같은 결과가 두 번 반영되는 것만 확실히 막는다.
completed_result_message_ids: set[int] = set()


def _get_record_lock(guild_id: int) -> asyncio.Lock:
    lock = record_locks.get(guild_id)
    if lock is None:
        lock = asyncio.Lock()
        record_locks[guild_id] = lock
    return lock
...
# 길드 ID -> 비공개 실제 티어를 저장해둘 채널 ID
secret_channels: dict[int, int] = {}
# 길드 ID -> 그 채널에 올려둔 비밀 티어 저장소 메시지 ID (캐시)
secret_message_ids: dict[int, int] = {}
# 길드 ID -> 비밀 티어 읽기/계산/저장을 한 번에 하나씩만 처리하게 만드는 잠금 (지갑/전적과 동일한 이유)
secret_locks: dict[int, asyncio.Lock] = {}

_SECRET_HEADER = "🔒 **비밀 티어 저장소**"


def _get_secret_lock(guild_id: int) -> asyncio.Lock:
    lock = secret_locks.get(guild_id)
    if lock is None:
        lock = asyncio.Lock()
        secret_locks[guild_id] = lock
    return lock
...
def _parse_wallet_json(data: bytes) -> dict[int, int]:
    try:
    ...
    if isinstance(raw, dict):
        for uid_str, bal in raw.items():
            try:
                balances[int(uid_str)] = int(bal)
            except (TypeError, ValueError):
                continue
    return balances


async def _load_balances(existing_msg: Optional[discord.Message]) -> dict[int, int]:
    """지갑 메시지에 붙어있는 balances.json 첨부파일을 읽어서 파싱한다."""
    if existing_msg is None or not existing_msg.attachments:
        return {}
    data = await existing_msg.attachments[0].read()
    return _parse_wallet_json(data)


def _build_wallet_file(balances: dict[int, int]) -> discord.File:
    sorted_balances = sorted(balances.items(), key=lambda kv: -kv[1])
    payload = {str(uid): bal for uid, bal in sorted_balances}
    buffer = io.BytesIO(json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8"))
    return discord.File(buffer, filename="balances.json")
...
async def _replace_wallet_message(
    ...
    실제 데이터는 balances.json 첨부파일에 담고, 본문은 짧은 캡션만 남긴다
    (본문에 전부 적으면 인원이 많아질수록 2,000자 제한에 걸리는 문제가 있었음).
    캡션에는 멘션을 안 쓰므로(파일 안에만 유저ID가 들어감) allowed_mentions 걱정도 자연히 없어짐."""
    old_msg = await _find_wallet_message(client, channel, guild_id)
    caption = f"{_WALLET_HEADER} (총 {len(balances)}명 · 아래 첨부파일이 실제 데이터예요)"
    new_msg = await channel.send(caption, file=_build_wallet_file(balances))
...
async def _settle_bet(guild_id: int, user_id: int, bet: int, outcome: str):
    ...
    async with _get_wallet_lock(guild_id):
        ...
        balances = await _load_balances(msg)
...
def _parse_record_json(data: bytes) -> dict[str, tuple[int, int]]:
    """records.json 첨부파일 내용을 승/패 딕셔너리로 변환. 형식이 깨져있으면 빈 값으로 처리(에러로 죽지 않게)."""
    try:
        raw = json.loads(data.decode("utf-8"))
    except (json.JSONDecodeError, UnicodeDecodeError):
        return {}
    ...
    if isinstance(raw, dict):
        for name, pair in raw.items():
            try:
                win, loss = pair
                records[name] = (int(win), int(loss))
            except (TypeError, ValueError):
                continue
    return records


async def _load_records(existing_msg: Optional[discord.Message]) -> dict[str, tuple[int, int]]:
    """전적 메시지에 붙어있는 records.json 첨부파일을 읽어서 파싱한다."""
    if existing_msg is None or not existing_msg.attachments:
        return {}
    data = await existing_msg.attachments[0].read()
    return _parse_record_json(data)


def _build_record_file(records: dict[str, tuple[int, int]]) -> discord.File:
    sorted_records = sorted(records.items(), key=lambda kv: (-kv[1][0], kv[0]))
    payload = {name: [win, loss] for name, (win, loss) in sorted_records}
    buffer = io.BytesIO(json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8"))
    return discord.File(buffer, filename="records.json")
...
async def _replace_record_message(
    ...
    실제 데이터는 records.json 첨부파일에 담고, 본문은 사람이 알아볼 짧은 캡션만 남긴다
    (메시지 본문에 전부 적으면 인원이 많아질수록 2,000자 제한에 걸리는 문제가 있었음)."""
    existing_msg = await _find_record_message(client, channel, guild_id)

    caption = f"{_RECORD_HEADER} (총 {len(records)}명 · 아래 첨부파일이 실제 데이터예요)"
    new_msg = await channel.send(
        caption, file=_build_record_file(records), allowed_mentions=discord.AllowedMentions.none()
    )
...
async def _apply_record_updates(
    ...
    async with _get_record_lock(guild_id):
        ...
        records = await _load_records(existing_msg)
...
async def _get_secret_channel(interaction: discord.Interaction):
    channel_id = secret_channels.get(interaction.guild_id)
    if channel_id is None:
        return None
...
async def _find_secret_message(client: discord.Client, channel: discord.abc.Messageable, guild_id: int):
    cached_id = secret_message_ids.get(guild_id)
    if cached_id is not None:
        try:
            return await channel.fetch_message(cached_id)
        except (discord.NotFound, discord.Forbidden):
    ...
    async for msg in channel.history(limit=50):
        if msg.author.id == client.user.id and msg.content.startswith(_SECRET_HEADER):
            secret_message_ids[guild_id] = msg.id
            return msg

    return None


def _parse_secret_text(content: str) -> dict[str, tuple[str, Optional[str]]]:
    """비밀 티어 저장소 메시지를 (닉네임 -> (티어, 세부단계))로 파싱한다.
    많은 인원을 넣을 게 아니라고 하셔서 전적/지갑과 달리 예전 방식(텍스트 본문)을 그대로 씀 —
    관리자가 채널에서 직접 눈으로 훑어보기에도 이 방식이 더 편함."""
    secrets: dict[str, tuple[str, Optional[str]]] = {}
    for line in content.splitlines():
        line = line.strip()
        if ": " not in line:
            continue
        name, tier_text = line.split(": ", 1)
        tier, division, error = parse_tier_text(tier_text.strip())
        if error is None:
            secrets[name] = (tier, division)
    return secrets


def _build_secret_text(secrets: dict[str, tuple[str, Optional[str]]]) -> str:
    lines_out = [_SECRET_HEADER, "", "(이 채널은 비공개로 운영해주세요 — 여기 적힌 티어는 밸런싱 계산에만 쓰이고,"]
    lines_out.append("신청 화면·팀 결과에는 항상 본인이 적어낸 티어만 표시됩니다.)")
    lines_out.append("")
    for name, (tier, division) in sorted(secrets.items(), key=lambda kv: kv[0]):
        lines_out.append(f"{name}: {tier_label(tier, division)}")
    return "\n".join(lines_out)


async def _replace_secret_message(
    client: discord.Client,
    channel: discord.abc.Messageable,
    guild_id: int,
    secrets: dict[str, tuple[str, Optional[str]]],
) -> None:
    """새 내용으로 새 메시지를 먼저 올리고, 성공하면 기존 비밀 저장소 메시지를 지운다 (전송 실패 시 데이터 유실 방지)."""
    existing_msg = await _find_secret_message(client, channel, guild_id)
    new_msg = await channel.send(_build_secret_text(secrets), allowed_mentions=discord.AllowedMentions.none())
    secret_message_ids[guild_id] = new_msg.id
...
async def _apply_secret_tiers(interaction: discord.Interaction, players: list[PlayerInfo]) -> None:
    """비밀창고 채널이 설정돼있으면 읽어와서, 이름이 일치하는 선수의 밸런싱 점수만 조용히 덮어쓴다.
    채널이 없거나 등록된 게 없으면 아무 일도 안 하고 조용히 지나감 (이 기능을 안 쓰는 서버에 영향 없게)."""
    channel = await _get_secret_channel(interaction)
    if channel is None:
        return
    existing_msg = await _find_secret_message(interaction.client, channel, interaction.guild_id)
    if existing_msg is None:
        return
    secrets = _parse_secret_text(existing_msg.content)
    if not secrets:
        return
    lookup = {name.strip().lower(): (tier, division) for name, (tier, division) in secrets.items()}
    for p in players:
        entry = lookup.get(p.name.strip().lower())
        if entry is not None:
            p.secret_tier, p.secret_division = entry
...
class WaitlistApplyModal(discord.ui.Modal, title="내전 대기자 신청"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if line1_value not in LINES or line2_value not in LINES:
            ...
            return
        if line1_value == line2_value:
            await interaction.response.send_message("주라인과 부라인은 서로 다르게 적어주세요.", ephemeral=True)
            return
...
class ResultModal(discord.ui.Modal, title="내전 결과 입력"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 여기서부터는 실제로 전적에 반영하기로 확정된 지점이라, await 없이(=끼어들 틈 없이)
        # 곧바로 완료 표시부터 남긴다. 두 사람이 거의 동시에 버튼→모달→제출까지 했더라도
        # 둘 중 실제로 이 줄을 먼저 통과하는 쪽만 반영되고, 나머지는 아래에서 거절된다.
        if self.source_message.id in completed_result_message_ids:
            await interaction.response.send_message(
                "이미 이 결과는 처리됐어요 (다른 관리자가 먼저 입력했을 수 있어요).", ephemeral=True
            )
            return
        completed_result_message_ids.add(self.source_message.id)
        ...
        try:
            ...
            if not applied:
                # 실제로 반영이 안 끝났으면(어떤 이유로든) 완료 표시를 되돌려서 재시도할 수 있게 함
                completed_result_message_ids.discard(self.source_message.id)
...
def _build_match_debug_text(match: dict) -> str:
    ...
    for team in ("A", "B"):
        ...
        for i in members:
            p = players[i]
            secret_note = ""
            if p.secret_tier is not None:
                secret_note = f" 🔒실제:{tier_label(p.secret_tier, p.secret_division)}"
            lines_out.append(f"- {line_assignment[i]}: {p.name} ({p.tier_text}){secret_note} [주:{p.line1}/부:{p.line2}]")
        lines_out.append("")
...
class PlayerLineEditModal(discord.ui.Modal, title="선호 라인 수정"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        match["players"][self.player_index] = PlayerInfo(
            ...
            secret_tier=p.secret_tier, secret_division=p.secret_division,
        )
...
async def start_recruitment_fill(interaction: discord.Interaction):
    ...
    recruitment_sessions[channel_id] = {"mode": "fill", "message_id": msg.id, "waitlist": waitlist}
...
async def make_teams_from_recruitment(interaction: discord.Interaction):
    ...
    # 비밀 티어 조회(채널/메시지 네트워크 호출)가 3초를 넘길 수 있어서 먼저 defer로 응답을 확정해둠
    await interaction.response.defer()

    await _apply_secret_tiers(interaction, players)
...
async def substitute_player(interaction: discord.Interaction, 기존닉네임: str, 새닉네임: str, 새티어: str):
    ...
    # 비밀 티어 조회가 3초를 넘길 수 있어서 먼저 defer로 응답을 확정해둠
    await interaction.response.defer(ephemeral=True)

    await _apply_secret_tiers(interaction, [new_players[idx]])
...
async def reshuffle_teams(interaction: discord.Interaction):
    ...
    # 1차 시도: 신청할 때 적어낸 주라인/부라인 선호도 + 지금까지 걸어둔 조건(팀묶기/팀분리/라인교체·라인요청)을
...
async def confirm_match(interaction: discord.Interaction):
    channel_id = interaction.channel_id
    # await 없이(=끼어들 틈 없이) 바로 꺼내서, 거의 동시에 두 번 확정해도 두 번째 호출은
    # 곧바로 "찾을 수 없음"을 보게 함 (안 그러면 결과채널에 같은 결과가 두 번 올라갈 수 있었음)
    match = active_matches.pop(channel_id, None)
    if match is None:
...
@bot.tree.command(
    name="비밀창고채널설정",
    description="[관리자] 밸런싱 계산에만 쓰일 비공개 실제 티어를 저장할 채널을 지정합니다.",
)
@app_commands.describe(채널="비밀 티어를 저장할 채널 (반드시 관리자만 볼 수 있는 채널로 만들어주세요)")
@app_commands.checks.has_permissions(manage_guild=True)
async def set_secret_channel(interaction: discord.Interaction, 채널: discord.TextChannel):
    secret_channels[interaction.guild_id] = 채널.id
    secret_message_ids.pop(interaction.guild_id, None)
    await interaction.response.send_message(
        f"이제부터 비밀 티어를 {채널.mention}에 저장할게요.\n"
        "⚠️ 이 채널은 신청자들이 못 보게 반드시 비공개 채널로 만들어주세요 — 봇이 채널 권한까지 대신 설정해주진 않아요.",
        ephemeral=True,
    )
...
@bot.tree.command(
    name="비밀추가",
    description="[관리자] 특정 닉네임의 밸런싱 계산용 비공개 실제 티어를 등록/수정합니다 (표기는 안 바뀜).",
)
@app_commands.describe(닉네임="대상 롤 닉네임", 티어="밸런싱 계산에만 쓰일 실제 티어 (예: 다이아1, 마스터)")
@app_commands.checks.has_permissions(manage_guild=True)
async def add_secret_tier(interaction: discord.Interaction, 닉네임: str, 티어: str):
    tier, division, error = parse_tier_text(티어)
    if error:
        await interaction.response.send_message(error, ephemeral=True)
        return
    ...
    channel = await _get_secret_channel(interaction)
    if channel is None:
        await interaction.response.send_message("먼저 /비밀창고채널설정으로 채널을 지정해주세요.", ephemeral=True)
        return

    # 조회+저장이 3초를 넘길 수 있어서 먼저 defer로 응답을 확정해둠
    await interaction.response.defer(ephemeral=True)

    async with _get_secret_lock(interaction.guild_id):
        existing_msg = await _find_secret_message(interaction.client, channel, interaction.guild_id)
        secrets = _parse_secret_text(existing_msg.content) if existing_msg else {}

        nickname_key = 닉네임.strip().lower()
        existing_key = next((k for k in secrets if k.strip().lower() == nickname_key), 닉네임)
        secrets[existing_key] = (tier, division)

        try:
            await _replace_secret_message(interaction.client, channel, interaction.guild_id, secrets)
        except discord.Forbidden:
            await interaction.followup.send(
                f"{channel.mention}에 메시지를 쓸 권한이 없어요. 봇 권한을 확인해주세요.", ephemeral=True
            )
            return

    await interaction.followup.send(
        f"'{existing_key}'의 비밀 티어를 {tier_label(tier, division)}(으)로 등록했어요. "
        "다음 /내전팀·/내전대타부터 밸런싱 계산에 반영돼요 (신청 화면·팀 결과 표기는 그대로예요).",
        ephemeral=True,
    )


@bot.tree.command(name="비밀삭제", description="[관리자] 등록해둔 비공개 실제 티어를 삭제합니다.")
@app_commands.describe(닉네임="삭제할 롤 닉네임")
@app_commands.checks.has_permissions(manage_guild=True)
async def delete_secret_tier(interaction: discord.Interaction, 닉네임: str):
    channel = await _get_secret_channel(interaction)
    if channel is None:
        await interaction.response.send_message("먼저 /비밀창고채널설정으로 채널을 지정해주세요.", ephemeral=True)
        return
    ...
    async with _get_secret_lock(interaction.guild_id):
        ...
        nickname_key = 닉네임.strip().lower()
        existing_key = next((k for k in secrets if k.strip().lower() == nickname_key), None)

        if existing_key is None:
            await interaction.followup.send(f"'{닉네임}'의 비밀 티어 기록을 찾을 수 없어요.", ephemeral=True)
            return

        del secrets[existing_key]
    ...
    await interaction.followup.send(f"'{existing_key}'의 비밀 티어 기록을 삭제했어요.", ephemeral=True)
...
async def search_record(interaction: discord.Interaction, 닉네임: str):
    ...
    # 메시지/첨부파일 조회가 3초를 넘길 수 있어서 먼저 defer로 응답을 확정해둠
    await interaction.response.defer()
    ...
    if found is None:
        await interaction.followup.send(f"'{닉네임}'의 전적 기록을 찾을 수 없어요.", ephemeral=True)
        return

    name, (win, loss) = found
    total = win + loss
    winrate = f"{win / total * 100:.1f}%" if total > 0 else "-"
    await interaction.followup.send(f"**{name}**: {win}승 {loss}패 (승률 {winrate})")
...
async def edit_record(interaction: discord.Interaction, 닉네임: str, 승: int, 패: int):
    ...
    async with _get_record_lock(interaction.guild_id):
        existing_msg = await _find_record_message(interaction.client, channel, interaction.guild_id)
        records = await _load_records(existing_msg)
    ...
    await interaction.followup.send(f"'{existing_key}' 전적을 {승}승 {패}패로 수정했어요.", ephemeral=True)
...
async def delete_record(interaction: discord.Interaction, 닉네임: str):
    ...
    await interaction.followup.send(f"'{existing_key}' 전적 기록을 삭제했어요.", ephemeral=True)
