...
async def on_member_remove(member: discord.Member):
    ...
    embed = discord.Embed(
        title="👋 서버 퇴장",
        description=f"**{member.display_name}**님이 서버를 나갔어요.",
...
async def _replace_wallet_message(
    ...
    게임 한 판마다 등록된 사람 전원의 멘션이 새 메시지로 다시 올라가면서 매번 알림이 가던 문제가 있어서,
    allowed_mentions=none으로 멘션 표시(파란 칩, 클릭 시 프로필 이동)는 그대로 두되 알림만 안 가게 함."""
    old_msg = await _find_wallet_message(client, channel, guild_id)
    text = _build_wallet_text(balances)
    new_msg = await channel.send(text, allowed_mentions=discord.AllowedMentions.none())
