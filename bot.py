...
- /내전라인교체, /내전팀묶기, /내전팀분리, /내전대타, /내전라인요청, /내전다시섞기, /내전위치교환 : /내전팀 이후 팀 결과 메시지를 다시 읽어서 조정
...
# ---------------------------------------------------------------------------
# /내전팀 결과가 이미 올라온 메시지를 읽고/고쳐서 팀 구성을 조정하는 기능
# (별도 메모리 상태 없이, 그 메시지 내용 자체를 매번 다시 파싱해서 사용함)
# ---------------------------------------------------------------------------

def _parse_tier_label_text(text: str) -> tuple[Optional[str], Optional[str], Optional[int]]:
    """tier_label()이 만든 표기("골드2", "마스터", "마스터 370" 등)를 역파싱한다."""
    text = text.strip()
    if " " in text:
    ...
    if base and base[-1] in DIVISIONS and base[:-1] in TIERS and base[:-1] not in _NO_DIVISION_SCORE:
        return base[:-1], base[-1], lp
    if base in TIERS:
        return base, None, lp
    return None, None, None


def _parse_match_state(content: str):
    """팀 결과 메시지 내용에서 (players, line_assignment, team_of)를 복원한다.
    이미 확정됐거나 형식이 안 맞으면 None을 반환한다."""
    if "확정됨" in content:
        return None
    ...
    for section, side in ((parts[0], "A"), (parts[1], "B")):
        for line, name, tier_text in pattern.findall(section):
            if line not in LINES:
                return None
            tier, division, lp = _parse_tier_label_text(tier_text)
            if tier is None:
                return None
            players.append(PlayerInfo(name=name, tier=tier, division=division, line1=line, line2=line, lp=lp))
    ...
    if len(players) != 10:
        return None
    return players, line_assignment, team_of


async def _find_active_match_message(client: discord.Client, channel: discord.abc.Messageable):
    """이 채널에서 아직 확정 안 된 가장 최근 팀 결과 메시지를 찾는다."""
    async for msg in channel.history(limit=50):
        if msg.author.id == client.user.id and "**🔵 팀 A**" in msg.content and "확정됨" not in msg.content:
            return msg
    return None
...
# (전부 이 채널에 있는, 아직 확정 안 된 가장 최근 팀 결과 메시지를 대상으로 함)
# ---------------------------------------------------------------------------

_NO_MATCH_MSG = "이 채널에서 수정 가능한 팀 결과를 찾을 수 없어요 (없거나 이미 확정됨)."
_UNPARSABLE_MSG = "팀 결과 메시지를 읽지 못했어요."
...
@bot.tree.command(name="내전라인교체", description="[관리자] 두 사람의 라인을 서로 바꾸고 팀 밸런스를 다시 맞춥니다.")
@app_commands.describe(닉네임A="라인을 바꿀 사람 1", 닉네임B="라인을 바꿀 사람 2")
@app_commands.checks.has_permissions(manage_guild=True)
async def swap_lines(interaction: discord.Interaction, 닉네임A: str, 닉네임B: str):
    msg = await _find_active_match_message(interaction.client, interaction.channel)
    if msg is None:
        await interaction.response.send_message(_NO_MATCH_MSG, ephemeral=True)
        return
    parsed = _parse_match_state(msg.content)
    if parsed is None:
        await interaction.response.send_message(_UNPARSABLE_MSG, ephemeral=True)
        return
    players, line_assignment, _team_of = parsed

    idx_a = _find_player_index(players, 닉네임A)
    idx_b = _find_player_index(players, 닉네임B)
    if idx_a is None or idx_b is None:
        missing = 닉네임A if idx_a is None else 닉네임B
    ...
    new_line_assignment = dict(line_assignment)
    new_line_assignment[idx_a], new_line_assignment[idx_b] = new_line_assignment[idx_b], new_line_assignment[idx_a]

    result = solve_team_split(players, new_line_assignment)
    if result is None:
        await interaction.response.send_message("라인을 바꾼 뒤 팀을 다시 나눌 방법을 찾지 못했어요.", ephemeral=True)
        return
    ...
    if not await _safe_edit_match_message(interaction, msg, format_result(players, team_a, team_b, score_a, score_b)):
        return
    await interaction.response.send_message(f"'{닉네임A}', '{닉네임B}'의 라인을 바꾸고 팀을 다시 맞췄어요.", ephemeral=True)


@bot.tree.command(name="내전팀묶기", description="[관리자] 지정한 사람들(2~5명)을 반드시 같은 팀으로 묶고 팀 밸런스를 다시 맞춥니다.")
...
async def group_same_team(
    ...
    for idx in indices:
        ...
        if line in seen_lines:
            other_name = players[seen_lines[line]].name
            this_name = players[idx].name
            await interaction.response.send_message(
                f"'{this_name}'과(와) '{other_name}'은(는) 둘 다 {line} 라인이라 같은 팀이 될 수 없어요 "
                "(한 라인은 항상 두 팀에 하나씩 나뉘어요).",
                ephemeral=True,
            )
            return
        seen_lines[line] = idx

    result = solve_team_split(players, line_assignment, constraints=[("same", indices)])
    if result is None:
        await interaction.response.send_message("이 조건을 만족하는 팀 조합을 찾지 못했어요.", ephemeral=True)
        return

    team_a, team_b, score_a, score_b = result
    if not await _safe_edit_match_message(interaction, msg, format_result(players, team_a, team_b, score_a, score_b)):
        return
    await interaction.response.send_message(f"{', '.join(names)}을(를) 같은 팀으로 묶고 팀을 다시 맞췄어요.", ephemeral=True)


@bot.tree.command(name="내전팀분리", description="[관리자] 지정한 두 사람을 반드시 다른 팀으로 나누고 팀 밸런스를 다시 맞춥니다.")
@app_commands.describe(닉네임A="분리할 사람 1", 닉네임B="분리할 사람 2")
@app_commands.checks.has_permissions(manage_guild=True)
async def separate_teams(interaction: discord.Interaction, 닉네임A: str, 닉네임B: str):
    msg = await _find_active_match_message(interaction.client, interaction.channel)
    if msg is None:
        await interaction.response.send_message(_NO_MATCH_MSG, ephemeral=True)
        return
    ...
    result = solve_team_split(players, line_assignment, constraints=[("diff", (idx_a, idx_b))])
    if result is None:
        await interaction.response.send_message("이 조건을 만족하는 팀 조합을 찾지 못했어요.", ephemeral=True)
        return
    ...
    if not await _safe_edit_match_message(interaction, msg, format_result(players, team_a, team_b, score_a, score_b)):
        return
    await interaction.response.send_message(f"'{닉네임A}', '{닉네임B}'를 다른 팀으로 나누고 팀을 다시 맞췄어요.", ephemeral=True)


@bot.tree.command(name="내전대타", description="[관리자] 한 명을 빼고 새 사람으로 교체한 뒤 팀 밸런스를 다시 맞춥니다.")
...
async def substitute_player(interaction: discord.Interaction, 기존닉네임: str, 새닉네임: str, 새티어: str):
    ...
    result = solve_team_split(new_players, line_assignment)
    if result is None:
        await interaction.response.send_message("대타를 넣은 뒤 팀을 다시 나눌 방법을 찾지 못했어요.", ephemeral=True)
        return

    team_a, team_b, score_a, score_b = result
    if not await _safe_edit_match_message(
        interaction, msg, format_result(new_players, team_a, team_b, score_a, score_b)
    ):
        return
    await interaction.response.send_message(f"'{기존닉네임}' 대신 '{새닉네임}'을(를) 넣고 팀을 다시 맞췄어요.", ephemeral=True)


@bot.tree.command(name="내전라인요청", description="[관리자] 한 사람을 원하는 라인으로 옮기고 팀 밸런스를 다시 맞춥니다.")
...
async def request_line(interaction: discord.Interaction, 닉네임: str, 라인: app_commands.Choice[str]):
    ...
    if line_assignment[idx] == target_line:
        await interaction.response.send_message(f"'{닉네임}'은(는) 이미 {target_line}이에요.", ephemeral=True)
        return

    occupants = [i for i, l in line_assignment.items() if l == target_line and i != idx]

    best = None
    for occ in occupants:
        trial = dict(line_assignment)
        trial[idx], trial[occ] = trial[occ], trial[idx]
        result = solve_team_split(players, trial)
        if result is None:
            continue
        diff = abs(result[2] - result[3])
        if best is None or diff < best[0]:
            best = (diff, occ, result)

    if best is None:
        await interaction.response.send_message("이동시킬 방법을 찾지 못했어요.", ephemeral=True)
        return

    _, swapped_idx, (team_a, team_b, score_a, score_b) = best
    swapped_name = players[swapped_idx].name
    original_line = line_assignment[idx]
    if not await _safe_edit_match_message(interaction, msg, format_result(players, team_a, team_b, score_a, score_b)):
        return
    await interaction.response.send_message(
        f"'{닉네임}'을(를) {target_line}(으)로 옮기고, 대신 '{swapped_name}'이(가) {original_line}(으)로 이동했어요. "
        "팀도 다시 맞췄어요.",
        ephemeral=True,
    )
...
async def reshuffle_teams(interaction: discord.Interaction):
    ...
    if parsed is None:
        await interaction.response.send_message(_UNPARSABLE_MSG, ephemeral=True)
        return
    players, line_assignment, team_of = parsed

    current_a_set = frozenset(i for i, t in team_of.items() if t == "A")
    result = solve_team_split(players, line_assignment, exclude_signature=current_a_set)
    if result is None:
        await interaction.response.send_message(
            "현재랑 다른 조합을 찾지 못했어요 (가능한 경우의 수가 다 똑같은 것 같아요).", ephemeral=True
        )
        return
    ...
    if not await _safe_edit_match_message(interaction, msg, format_result(players, team_a, team_b, score_a, score_b)):
        return
    await interaction.response.send_message("팀을 다른 조합으로 다시 나눴어요. (라인 배정은 그대로 유지했어요)", ephemeral=True)


@bot.tree.command(name="내전위치교환", description="[관리자] 밸런스 재계산 없이 두 사람의 팀+라인을 그대로 맞바꿉니다.")
@app_commands.describe(닉네임A="교환할 사람 1", 닉네임B="교환할 사람 2")
@app_commands.checks.has_permissions(manage_guild=True)
async def swap_positions(interaction: discord.Interaction, 닉네임A: str, 닉네임B: str):
    msg = await _find_active_match_message(interaction.client, interaction.channel)
    if msg is None:
        await interaction.response.send_message(_NO_MATCH_MSG, ephemeral=True)
        return
    ...
    if not await _safe_edit_match_message(interaction, msg, format_result(players, team_a, team_b, score_a, score_b)):
        return
    await interaction.response.send_message(
        f"'{닉네임A}', '{닉네임B}'의 자리를 그대로 맞바꿨어요 (밸런스 재계산 없음).", ephemeral=True
    )
